<?php 
  ...
  if(isset($_GET['language']))
  {
      ...
      if(isset($_GET['genre'])){
        $query = "SELECT COUNT(*) as num FROM $tableName WHERE tbl_rentmovies.`language_id`='$lang_id' AND FIND_IN_SET(".$_GET['genre'].", tbl_rentmovies.`genre_id`)";

        $targetpage = "manage_rentmovies.php?language=$lang_id&genre=".$_GET['genre']; 
      }
      else{
        $query = "SELECT COUNT(*) as num FROM $tableName WHERE tbl_rentmovies.`language_id`='$lang_id'";

        $targetpage = "manage_rentmovies.php?language=$lang_id";   
      }
      ...
      $sql="SELECT tbl_language.`language_name`,tbl_rentmovies.* FROM tbl_rentmovies
          LEFT JOIN tbl_language ON tbl_rentmovies.`language_id`= tbl_language.`id` 
          WHERE tbl_rentmovies.`language_id`='$lang_id' and tbl_rentmovies.`movie_cost_type`='paid'
          ORDER BY tbl_rentmovies.`id` DESC LIMIT $start, $limit";
      ...
      if(isset($_GET['genre'])){
        ...
        $sql="SELECT tbl_language.`language_name`,tbl_rentmovies.* FROM tbl_rentmovies
            LEFT JOIN tbl_language ON tbl_rentmovies.`language_id`= tbl_language.`id` 
            WHERE tbl_rentmovies.`language_id`='$lang_id' AND FIND_IN_SET($genre_id, tbl_rentmovies.`genre_id`) and tbl_rentmovies.`movie_cost_type`='paid'
            ORDER BY tbl_rentmovies.`id` DESC LIMIT $start, $limit";
      }
  ...
  else if(isset($_GET['genre']))
  {
      ...
      $query = "SELECT COUNT(*) as num FROM $tableName WHERE FIND_IN_SET(".$genre_id.", tbl_rentmovies.`genre_id`)";

      $targetpage = "manage_rentmovies.php?genre=".$genre_id; 
      ...
      $sql="SELECT tbl_language.`language_name`,tbl_rentmovies.* FROM tbl_rentmovies
          LEFT JOIN tbl_language ON tbl_rentmovies.`language_id`= tbl_language.`id` 
          WHERE FIND_IN_SET(tbl_rentmovies.`genre_id`,$genre_id) and tbl_rentmovies.`movie_cost_type`='paid'
          ORDER BY tbl_rentmovies.`id` DESC LIMIT $start, $limit";
  ...
  else if(isset($_POST["search"]))
  {
    ...
    $sql="SELECT tbl_language.`language_name`,tbl_rentmovies.* FROM tbl_rentmovies
          LEFT JOIN tbl_language ON tbl_rentmovies.`language_id`= tbl_language.`id` 
          WHERE (tbl_rentmovies.`movie_title` LIKE '%$search_txt%' OR tbl_language.`language_name` LIKE '%$search_txt%') and tbl_rentmovies.`movie_cost_type`='paid'
          ORDER BY tbl_rentmovies.`id` DESC";
     
  }
  else
  { 
    
    $targetpage = "manage_rentmovies.php"; 
      ...
      <div class="page_title_block">
        ...
        <div class="col-md-7 col-xs-12">              
          <div class="search_list">
            ...
            <div class="add_btn_primary"> <a href="add_rentmovies.php?add=yes">Add Rent Movie &nbsp;<i class="fa fa-plus-circle"></i></a> </div>
          </div>
        </div>
            ...
            <div class="block_wallpaper" style"width:251px;height:315px;flex-shrink: 0;border-radius:550px;">
              ...
              <div class="wall_image_title" style="background: rgba(0,0,0,0.0);border-radius:20px;">
               ...
               <ul>
                  ...
                  <li><a href="add_rentmovies.php?movie_id=<?php echo $row['id'];?>&redirect=<?=$redirectUrl?>"  style="background:linear-gradient(91.59deg, rgb(148, 63, 213) 1.37%, rgb(97, 43, 173) 98.71%);width:74px;border-radius:4px;" data-toggle="tooltip" data-tooltip="Edit"> Edit <i class="fa fa-edit"></i></a></li>
                  <?php if($row['status']!="0"){?>
