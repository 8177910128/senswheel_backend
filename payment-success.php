...
if ($status == "success") {
    ...
    $check_txn = "SELECT id FROM tbl_payment_history WHERE txnid = '$txnid' and payu_id='$payu_id' ";
    $check_result = mysqli_query($mysqli, $check_txn);

    if (mysqli_num_rows($check_result) == 0) {
        // Insert payment record
        $insert = "INSERT INTO tbl_payment_history 
            (user_id, plan_id, txnid, amount, payu_id, status, payment_time) 
            VALUES 
            ('$userid', '$plan_id', '$txnid', '$amount', '$payu_id', '$status', '$payment_time')";

        if (mysqli_query($mysqli, $insert)) {
            $date = new DateTime("now", new DateTimeZone("Asia/Kolkata"));
            $dd = $date->format('Y-m-d H:i:s'); // 24-hour format
            ...
            $plan_validation = $plan_row['validity'];

            $update = "UPDATE tbl_users 
                       SET plan_id = '$plan_id', plan_expire = '0', 
                           plan_buy_date = '$dd', plan_validation = '$plan_validation' 
                       WHERE id = '$userid'";
            ...
            <h1><strong>Plan Subscribed Successfully</strong> </h1>
