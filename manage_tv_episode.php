<?php 
  ...
  if(isset($_GET['shows']))
  {
      ...
      if(isset($_GET['season'])){
        ...
        $query = "SELECT COUNT(*) as num FROM $tableName WHERE tbl_tv_episode.`shows_id`='$shows_id' AND tbl_tv_episode.`season_id`='$season_id'";

        $targetpage = "manage_tv_episode.php?shows=$shows_id&season=".$season_id; 
      }
      else{
        $query = "SELECT COUNT(*) as num FROM $tableName WHERE tbl_tv_episode.`shows_id`='$shows_id'";

        $targetpage = "manage_tv_episode.php?shows=$shows_id";   
      }
      ...
      $sql="SELECT episode.*, shows.`shows_name`, season.`season_name`  FROM tbl_tv_episode episode
            LEFT JOIN tbl_shows shows ON episode.`shows_id`=shows.`id`
            LEFT JOIN tbl_tv_season season ON episode.`season_id`=season.`id`
            WHERE episode.`shows_id`='$shows_id'
      ...
      if(isset($_GET['season'])){
        ...
        $sql="SELECT episode.*, shows.`shows_name`, season.`season_name`  FROM tbl_tv_episode episode
            LEFT JOIN tbl_shows shows ON episode.`shows_id`=shows.`id`
            LEFT JOIN tbl_tv_season season ON episode.`season_id`=season.`id`
            WHERE episode.`shows_id`='$shows_id' AND episode.`season_id`='$season_id'
            ORDER BY episode.`id` DESC LIMIT $start, $limit";
      }
  ...
  else if(isset($_POST['data_search']))
  {
      ...
      $sql="SELECT episode.*, shows.`shows_name`, season.`season_name`  FROM tbl_tv_episode episode
          LEFT JOIN tbl_shows shows ON episode.`shows_id`=shows.`id`
          LEFT JOIN tbl_tv_season season ON episode.`season_id`=season.`id`
          WHERE (season.`season_name` LIKE '%$search_txt%' OR episode.`episode_title` LIKE '%$search_txt%' OR shows.`shows_name` LIKE '%$search_txt%')
      ...
      <div class="page_title_block">
        ...
        <div class="col-md-7 col-xs-12">
          <div class="search_list">
            ...
            <div class="add_btn_primary"> <a href="add_tv_episode.php?add=yes">Add Episode &nbsp;<i class="fa fa-plus-circle"></i></a> </div>
          </div>
        </div>
        ...
        <form id="filterForm" accept="" method="GET">
          <div class="col-md-3">
            <div class="" style="padding: 0px 0px 5px;">
              <select name="shows" class="form-control select2 filter" style="padding: 5px 10px;height: 40px;">
                  <option value=""> Shows </option>
                  <?php 
                    $sql="SELECT * FROM tbl_shows ORDER BY id DESC";
                    $res_shows=mysqli_query($mysqli, $sql) or die(mysqli_error($mysqli));
                    while ($info=mysqli_fetch_assoc($res_shows)) {
                      ?>
                      <option value="<?=$info['id']?>" <?php if(isset($_GET['shows']) && $_GET['shows']==$info['id']){ echo 'selected';} ?>><?=$info['shows_name']?></option>
                      <?php
                    }
                    mysqli_free_result($res_shows);
                  ?>
                </select>
            </div>
          </div>
          <div class="col-md-3">
            <div class="" style="padding: 0px 0px 5px;">
              <select name="season" class="form-control select2 filter" style="padding: 5px 10px;height: 40px;">
                  ...
                  <?php
                    if(isset($_GET['shows'])){
                      $sql="SELECT * FROM tbl_tv_season WHERE `shows_id`='".$_GET['shows']."' ORDER BY `season_name` ASC";
      ...
      <div class="col-md-12 mrg-top">
        <div class="row">
          ...
          <div class="col-lg-3 col-sm-6 col-xs-12">
            <div class="block_wallpaper" style="border-radius:10px;">
              ...
              <div class="wall_image_title" style="background-color:rgba(0, 0, 0, 0.4);">
                ...
                <ul style="z-index: 1">
                    ...
                    <li><a href="" class="btn_delete_a" style="background:gray;width:74px;border-radius:4px;padding20px;" data-id="<?php echo $row['id'];?>" data-toggle="tooltip" data-tooltip="Delete">Delete <i class="fa fa-trash"></i></a></li>
                  <li><a href="add_tv_episode.php?episode_id=<?php echo $row['id'];?>&redirect=<?=$redirectUrl?>" style="background:linear-gradient(91.59deg, rgb(148, 63, 213) 1.37%, rgb(97, 43, 173) 98.71%);;width:74px;border-radius:4px;" data-toggle="tooltip" data-tooltip="Edit">Edit <i class="fa fa-edit"></i></a></li>
              ...
              <span><img src="<?php echo $row['episode_poster'];?>" /></span>
            </div>
          </div>
      <?php
