<?php 
  if(isset($_GET['drama_id'])){ 
    $page_title= 'Edit Drama';
  }
  else{ 
    $page_title='Add Drama'; 
  }
  $current_page="drama";
  $active_page="drama";
  ...
  function quality_info_data($post_id, $param, $type='drama'){

    global $mysqli;

    $sql="SELECT * FROM tbl_quality WHERE `post_id` = '$post_id' AND `type` = 'drama'";
  ...
  $cat_result=mysqli_query($mysqli,$cat_qry);
	...
	{	
      $title=addslashes(trim($_POST['drama_title']));
      $desc=addslashes(trim($_POST['drama_desc']));
      ...
      if($_POST['video_file_type']=='youtube_url'){
        ...
        $drama_url=$_POST['drama_url'];
        $youtube_video_url = addslashes($_POST['drama_url']);
      ...
      else if($_POST['video_file_type']=='server_url' OR $_POST['video_file_type']=='embedded_url')
      {
        ...
        $drama_url=date('dmYhis').'_'.rand(0,99999)."_drama".".".$ext;

        $tmp = $_FILES['video_local']['tmp_name'];
        
        if (move_uploaded_file($tmp, $path.$drama_url)) 
        {
          $drama_url=$drama_url;
        } else {
      ...
      if($_POST['poster_img']=='' || $_FILES['drama_poster']['error']!=4){

          // for dramas poster
          $drama_poster=rand(0,99999)."_".$_FILES['drama_poster']['name'];
          $pic1=$_FILES['drama_poster']['tmp_name'];
 
          $tpath1='images/dramas/'.$drama_poster; 
          copy($pic1,$tpath1);

          $thumbpath='images/dramas/thumbs/'.$drama_poster;
          ...
          $drama_poster=date('dmYhis').'_'.rand(0,99999).".".$ext;
      ...
      // for dramas cover
      $drama_cover=rand(0,99999)."_".$_FILES['drama_cover']['name'];
      $pic1=$_FILES['drama_cover']['tmp_name'];

            
      $tpath1='images/dramas/'.$drama_cover; 
      copy($pic1,$tpath1);

      $thumbpath='images/dramas/thumbs/'.$drama_cover;
      ...
      $data = array( 
        ...
        'drama_type'  =>  $_POST['video_file_type'],
        'drama_title'  =>  $title,
        'release_date'  =>  $release_date,
        'total_time'  =>  $total_time,
        'director_name'  =>  $director_name,
        'maturity_rating'  =>  $maturity_rating,
        'drama_cover'  =>  $mpath.$drama_cover,
        'drama_poster'  =>  $mpath.$drama_poster,
        'drama_url'  =>  $drama_url,
        'video_id'  =>  $video_id,
        'drama_desc'  =>  $desc,
      ...
      header( "Location:manage_drama.php");
      exit;
	}

  if(isset($_GET['drama_id']))
  {  
    $qry="SELECT * FROM tbl_drama where id='".$_GET['drama_id']."'";
    $result=mysqli_query($mysqli,$qry);
    $row=mysqli_fetch_assoc($result);
  }
  if(isset($_POST['submit']) and isset($_GET['drama_id']))
  { 
    ...
    if($_POST['video_file_type']=='youtube_url'){
      ...
      if($row['drama_type']=='local'){
        unlink('uploads/'.$row['drama_url']);
      }
      ...
      if($_FILES['video_local']['error']!=4){
        ...
        $drama_url=rand(0,99999)."_".str_replace(" ", "-", $_FILES['video_local']['name']);
    ...
    // for drama poster
    if($_FILES['drama_poster']['error']!=4){

      unlink('images/dramas/'.$row['drama_poster']);
      unlink('images/dramas/thumbs/'.$row['drama_poster']);

      // for drama poster
      $drama_poster=rand(0,99999)."_".$_FILES['drama_poster']['name'];
      $pic1=$_FILES['drama_poster']['tmp_name'];
            
      $tpath2='images/dramas/'.$drama_poster; 
      copy($pic1,$tpath2);
      ...
      $tpath2=$row['drama_poster'];
    }

    // for drama cover
    if($_FILES['drama_cover']['error']!=4){
      unlink('images/dramas/'.$row['drama_cover']);
      unlink('images/dramas/thumbs/'.$row['drama_cover']);
    ...
    $edit=Update('tbl_drama', $data, "WHERE id = '".$_GET['drama_id']."'");
    ...
    if(isset($_POST['quality_status']) && $_POST['quality_status']=='true'){
      ...
      if($_POST['video_file_type']=='local'){
        ...
        if(quality_info_data($_GET['drama_id'], 'post_upload_type')=='server_url'){
          $data = array( 
            'quality_480'  =>  '',
            'quality_720'  =>  '',
            'quality_1080'  =>  '',
          ...
          $updateSql=Update('tbl_quality', $data, "WHERE `post_id` = '".$_GET['drama_id']."' AND `type`='drama'");
        }
        ...
        foreach($quality_arr as $key => $value) {
            ...
            if($_FILES['quality_local']['error'][$value]!=4){
                ...
                unlink('uploads/'.quality_info_data($_GET['drama_id'], $param));
                ...
                $video_url=quality_info_data($_GET['drama_id'], $param);
            }
        ...
        if(quality_info_data($_GET['drama_id'], 'id')!=''){
          $data = array( 
            'post_upload_type'  =>  'local',
            'quality_480'  =>  $quality_480,
            'quality_720'  =>  $quality_720,
        ...
        if(quality_info_data($_GET['drama_id'], 'post_upload_type')=='local'){
            
            unlink('uploads/'.quality_info_data($_GET['drama_id'], 'quality_480'));
            unlink('uploads/'.quality_info_data($_GET['drama_id'], 'quality_720'));
            unlink('uploads/'.quality_info_data($_GET['drama_id'], 'quality_1080'));
        }
    ...
    if(isset($_GET['redirect']))
      ...
      header( "Location:add_drama.php?drama_id=".$_GET['drama_id']);
    exit;
    ...
    <div class="card">
      <div class="page_title_block">
        ...
        <div class="col-md-7 col-xs-12" style="text-align:right;">
           <a href="manage_drama.php" style="background:white;margin-top:10px;font-size:25px;">X</a>
        </div>
      </div>
      ...
      <div class="card-body mrg_bottom">
        ...
        <form class="form form-horizontal" action="" method="post"  enctype="multipart/form-data">
          ...
          <input type="hidden" name="drama_id" value="<?=(isset($_GET['drama_id'])) ? $_GET['drama_id'] : ''?>">

          <input type="hidden" name="is_quality" value="<?=(isset($_GET['drama_id'])) ? $row['is_quality'] : ''?>">

          <input type="hidden" name="is_subtitle" value="<?=(isset($_GET['drama_id'])) ? $row['subtitle'] : 'false'?>">
          ...
          <div class="section">
            <div class="section-body">
                ...
                <div class="col-md-8">
                  <select name="language_id" id="language_id" class="select2" required>
                    ...
                    <?php
                        ...
                        {
                    ?>                       
                    <option value="<?php echo $data['id'];?>" <?php if(isset($_GET['drama_id']) && $row['language_id']==$data['id']){ echo 'selected'; } ?>><?php echo $data['language_name'];?></option>                          
                    <?php
                      }
                    ?>
                  </select>
              ...
              <div class="form-group">
                <label class="col-md-3 control-label">Drama Title :-</label>
                <div class="col-md-8">
                  <input type="text" name="drama_title" placeholder="Enter drama Title" id="drama_title" value="<?php if(isset($_GET['drama_id'])){echo $row['drama_title'];}?>" class="form-control" required>
                </div>
              </div>
               <div class="form-group">
                <label class="col-md-3 control-label">Drama Director Name :-</label>
                <div class="col-md-8">
                  <input type="text" name="director_name" placeholder="Enter Director Name" id="director_name" value="<?php if(isset($_GET['drama_id'])){echo $row['director_name'];}?>" class="form-control" required>
                </div>
              </div>
              <div class="form-group">
                <label class="col-md-3 control-label">Casts :-</label>
                <div class="col-md-8">
                  <input type="text" name="cast_names" placeholder="Enter Casts" id="total_time" value="<?php if(isset($_GET['drama_id'])){echo $row['cast_names'];}?>" class="form-control" required>
                </div>
              </div>
              <div class="form-group">
                <label class="col-md-3 control-label">Imdb Rating :-</label>
                <div class="col-md-8">
                  <input type="text" name="imdb_rating" placeholder="Enter imdb rating" id="imdb_rating" value="<?php if(isset($_GET['drama_id'])){echo $row['imdb_rating'];}?>" class="form-control" required>
                </div>
                  </div>
              <div class="form-group">
                <label class="col-md-3 control-label">Relsease Date :-</label>
                <div class="col-md-8">
                  <input type="date" name="release_date" placeholder="date" id="release_date" value="<?php if(isset($_GET['drama_id'])){echo $row['release_date'];}?>" class="form-control" required>
                </div>
              </div>
              ...
              <div class="form-group">
                ...
                <div class="col-md-8">                       
                  <select name="video_file_type" id="video_file_type" class="select2" required>
                        ...
                        <option value="youtube_url" <?php if(isset($_GET['drama_id']) && $row['drama_type']=='youtube_url'){ echo 'selected'; } ?>>YouTube URL</option>
                        <option value="server_url" <?php if(isset($_GET['drama_id']) && $row['drama_type']=='server_url'){ echo 'selected'; } ?>>Live URL</option>
                        <option value="local" <?php if(isset($_GET['drama_id']) && $row['drama_type']=='local'){ echo 'selected'; } ?>>Local System</option>
                        <option value="embedded_url" <?php if(isset($_GET['drama_id']) && $row['drama_type']=='embedded_url'){ echo 'selected'; } ?>>Embedded URL (Open Load, Very Stream, Daily motion, Vimeo)</option>
              ...
              <div id="drama_url_holder" class="form-group" style="display:none;">
                ...
                <div class="col-md-8">
                  <input type="text" name="drama_url" value="<?php if(isset($_GET['drama_id'])){ echo $row['drama_url'];}?>" class="form-control">
                </div>
              </div>
              <div id="drama_local_holder" class="form-group" style="display:none;">
                ...
                <div class="col-md-8">
                  <input type="file" name="video_local" value="" class="form-control video_local">
                    <?php 
                      if(isset($_GET['drama_id']) && $row['is_quality']=='false'){
                     ?> 
                      <div style="word-break: break-all;"><label class="control-label">Current URL :-</label><?php echo $file_path.'uploads/'.$row['drama_url']?></div><br>
                      <?php  
                    }
                    ?>
                    <div class="uploadPreview" style="<?php if(isset($_GET['drama_id']) && $row['drama_type']=='local'){ echo 'display: block;';}else{ echo 'display: none;';}?>background: #eee;text-align: center;">
                      <video height="250" width="100%" class="video-preview" src="<?php if(isset($_GET['drama_id']) && $row['drama_type']=='local'){ echo $file_path.'uploads/'.$row['drama_url']; } ?>" controls="controls"/>
                    </div>
                </div>
              </div>
              ...
              <div id="quality_holder" style="display: none">
                ...
                <div class="form-group">
                  ...
                  <div class="col-md-8">                       
                    <select name="quality_status" id="quality_status" class="select2">
                      <option value="false" <?=(isset($_GET['drama_id']) && $row['is_quality']=='false') ? 'selected="selected"' : ''?>>OFF</option>
                      <option value="true"<?=(isset($_GET['drama_id']) && $row['is_quality']=='true') ? 'selected="selected"' : ''?>>ON</option>
                    </select>
                  </div>
                </div>
                <div id="quality_server_url" style="display: none;">
                  <div class="form-group">
                    ...
                    <div class="col-md-8">
                      <input type="text" name="quality_url[480]" value="<?=(isset($_GET['drama_id']) && $row['is_quality']=='true') ? quality_info_data($row['id'], 'quality_480') : ''?>" class="form-control">
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="col-md-3 control-label">Enter URL for 720p :-</label>
                    <div class="col-md-8">
                      <input type="text" name="quality_url[720]" value="<?=(isset($_GET['drama_id']) && $row['is_quality']=='true') ? quality_info_data($row['id'], 'quality_720') : ''?>" class="form-control">
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="col-md-3 control-label">Enter URL for 1080p :-</label>
                    <div class="col-md-8">
                      <input type="text" name="quality_url[1080]" value="<?=(isset($_GET['drama_id']) && $row['is_quality']=='true') ? quality_info_data($row['id'], 'quality_1080') : ''?>" class="form-control">
                    </div>
                  </div>
                </div>
                <div id="quality_local" style="display: none;">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-md-8">
                      ...
                      <?php 
                        if(isset($_GET['drama_id']) && $row['is_quality']=='true' && $row['drama_type']=='local' && quality_info_data($row['id'], 'quality_480')!=''){
                       ?> 
                      ...
                      ?>
                      <div class="uploadPreview" style="<?php if(isset($_GET['drama_id']) && $row['drama_type']=='local' && $row['is_quality']=='true' && quality_info_data($row['id'], 'quality_480')!=''){ echo 'display: block;';}else{ echo 'display: none;';}?>background: #eee;text-align: center;">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-md-8">
                      ...
                      <?php 
                        if(isset($_GET['drama_id']) && $row['is_quality']=='true' && $row['drama_type']=='local' && quality_info_data($row['id'], 'quality_720')!=''){
                       ?> 
                      ...
                      ?>
                      <div class="uploadPreview" style="<?php if(isset($_GET['drama_id']) && $row['drama_type']=='local' && $row['is_quality']=='true' && quality_info_data($row['id'], 'quality_720')!=''){ echo 'display: block;';}else{ echo 'display: none;';}?>background: #eee;text-align: center;">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-md-8">
                      ...
                      <?php 
                        if(isset($_GET['drama_id']) && $row['is_quality']=='true' && $row['drama_type']=='local' && quality_info_data($row['id'], 'quality_1080')!=''){
                       ?> 
                      ...
                      ?>
                      <div class="uploadPreview" style="<?php if(isset($_GET['drama_id']) && $row['drama_type']=='local' && $row['is_quality']=='true' && quality_info_data($row['id'], 'quality_1080')!=''){ echo 'display: block;';}else{ echo 'display: none;';}?>background: #eee;text-align: center;">
              ...
              <div class="subtitle" style="margin: 10px 0px;display: none;">
                <div class="form-group">
                  ...
                  <div class="col-md-8">                       
                    <select name="subtitle" id="subtitle" class="select2">
                      <option value="false" <?=(isset($_GET['drama_id']) && $row['subtitle']=='false') ? 'selected="selected"' : ''?>>OFF</option>
                      <option value="true"<?=(isset($_GET['drama_id']) && $row['subtitle']=='true') ? 'selected="selected"' : ''?>>ON</option>
                    </select>
                  </div>
                </div>
                ...
                <?php }else if(isset($_GET['drama_id']))
                {

                ?>
                <div class="subtitle_fields" style="display: none">
                <?php
                    $sql_subtitle="SELECT * FROM tbl_subtitles WHERE post_id='".$row['id']."' AND `type`='drama'";
              ...
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <input type="text" name="total_time" placeholder="Enter video time" id="total_time" value="<?php if(isset($_GET['drama_id'])){echo $row['total_time'];}?>" class="form-control" required>
                </div>
              </div>
               <div class="form-group">
                <label class="col-md-3 control-label">Maturity rating :-</label>
                <div class="col-md-8">
                  <input type="text" name="maturity_rating" placeholder="Enter maturity rating" id="maturity_rating" value="<?php if(isset($_GET['drama_id'])){echo $row['maturity_rating'];}?>" class="form-control" required>
                </div>
              </div>
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <div class="fileupload_block" style="border-radius:10px;height:189px;">
                    <input type="file" name="drama_poster" value="" accept=".png, .jpg, .jpeg, .svg, .gif" <?php echo (!isset($_GET['drama_id'])) ? 'required="require"' : '' ?> id="fileupload">
                    <div class="fileupload_img">
                      <?php 
                        $img_src="";

                        if(!isset($_GET['drama_id']) || !file_exists($row['drama_poster'])){
                          $img_src='assets/images/browse.svg';
                          echo '<input type="hidden" name="poster_img" value="">';
                        }else{
                          $img_src=$row['drama_poster'];
                        }
              ...
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <div class="fileupload_block" style="border-radius:10px;height:189px;">
                    <input type="file" name="drama_cover" value="" accept=".png, .jpg, .jpeg, .svg, .gif" <?php echo (!isset($_GET['drama_id'])) ? 'required="require"' : '' ?> id="fileupload" style="margin-top: 5%;">
                    <div class="fileupload_img">
                      <?php 
                        $img_src="";

                        if(!isset($_GET['drama_id']) || !file_exists($row['drama_cover'])){
                          $img_src='assets/images/browse.svg';  

                        }else{
                          $img_src=$row['drama_cover'];
                        }
              ...
              <div class="form-group">
                <div class="col-md-3">
                  <label class="control-label">drama Description :-</label>
                </div>
                <div class="col-md-8">
                  <textarea name="drama_desc" id="drama_desc" rows="5" class="form-control"><?php if(isset($_GET['drama_id'])){ echo $row['drama_desc']; } ?></textarea>
                  <script>
                    CKEDITOR.replace('drama_desc');
                  </script>
                </div>
              </div>
              ...
              <div class="form-group">
                <div class="col-md-9 col-md-offset-3">
                  <button type="submit" name="submit" style="border-radius:10px;" class="btn btn-primary"><?php if(isset($_GET['drama_id'])){ ?> Update <?php }else{?>Save<?php } ?> </button>
                </div>
              </div>
            </div>
          </div>
...
<script type="text/javascript">
  ...
  function readURL(input) {
    if (input.files && input.files[0]) {
      ...
      reader.onload = function(e) {
        $("input[name='drama_poster']").next(".fileupload_img").find("img").attr('src', e.target.result);
      }
      
      reader.readAsDataURL(input.files[0]);
    }
  }
  $("input[name='drama_poster']").change(function() { 
  ...
  function readURL1(input) {
    if (input.files && input.files[0]) {
      ...
      reader.onload = function(e) {
        $("input[name='drama_cover']").next(".fileupload_img").find("img").attr('src', e.target.result);
      }
  ...
  $("input[name='drama_cover']").change(function() { 
    var file=$(this);
  ...
  $("#video_file_type").on("change",function(e){
    ...
    if(_type=='youtube_url' || _type=='server_url' || _type=='embedded_url'){
      $("#drama_url_holder").show();
      $("input[name='video_local']").attr("required",false);
      $("input[name='drama_url']").attr("required",true);
      $("#drama_local_holder").hide();

    }
    else if(_type=='local'){
      $("input[name='drama_url']").attr("required",false);
      $("input[name='video_local']").attr("required",true);
      $("#drama_local_holder").show();
      $("#drama_url_holder").hide();
    }
    else{
      $("#quality_holder").hide();
      $("#drama_local_holder").hide();
  ...
  $("select[name='subtitle_type[]']").change(function(e){
    ...
    if(val==='live_url'){
      ...
      if($("input[name='drama_id']").val()=='' && $(this).parents(".subtitle_container").find("input[name='old_subtitle_type']").val()!='local'){
        ...
        if(subtitle_type==''){
          ...
          if(subtitle_type=='live_url'){
            ...
            if($(this).parents('.moreContainer').html()==undefined){
                ...
                if(file[0].files.length == 0 && ($("input[name='drama_id']").val()=='' || $("input[name='is_subtitle']").val()=='false')){
  ...
  $(".removeSubtitle").click(function(event){
    ...
    var post_id=$("input[name='drama_id']").val();
    ...
    if(confirm("Do you really want to remove this subtitle?")){
        $.ajax({
          ...
          url:'processData.php',
          dataType:'json',
          data:{id:id,'action':'remove_subtitle','type':'drama','post_id':post_id},
  ...
  $(".btn_fetch").click(function(e){
    ...
    if(imdb_id_title==''){
      ...
      $.ajax({
        ...
        url:'processImdb.php',
        dataType:'json',
        data:{'action':'getdramaDetails', 'id' : imdb_id_title},
        success:function(res){
            ...
            if(res.status=='1'){
                ...
                $("input[name='drama_poster']").attr("required",false);
                $("input[name='drama_title']").val(res.title);
                ...
                $("textarea[name='drama_desc']").val(res.plot);
                CKEDITOR.instances['drama_desc'].setData(res.plot);
            }
          }
      });
