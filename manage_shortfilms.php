<?php 
  ...
  if(isset($_GET['language']))
  {
      ...
      if(isset($_GET['genre'])){
        $query = "SELECT COUNT(*) as num FROM $tableName WHERE tbl_shortfilms.`language_id`='$lang_id' AND FIND_IN_SET(".$_GET['genre'].", tbl_shortfilms.`genre_id`) ";

        $targetpage = "manage_shortfilms.php?language=$lang_id&genre=".$_GET['genre']; 
      }
      else{
        $query = "SELECT COUNT(*) as num FROM $tableName WHERE tbl_shortfilms.`language_id`='$lang_id'";

        $targetpage = "manage_shortfilms.php?language=$lang_id";   
      }
      ...
      $sql="SELECT tbl_language.`language_name`,tbl_shortfilms.* FROM tbl_shortfilms
          LEFT JOIN tbl_language ON tbl_shortfilms.`language_id`= tbl_language.`id` 
          WHERE tbl_shortfilms.`language_id`='$lang_id' 
          ORDER BY tbl_shortfilms.`id` DESC LIMIT $start, $limit";
      ...
      if(isset($_GET['genre'])){
        ...
        $sql="SELECT tbl_language.`language_name`,tbl_shortfilms.* FROM tbl_shortfilms
            LEFT JOIN tbl_language ON tbl_shortfilms.`language_id`= tbl_language.`id` 
            WHERE tbl_shortfilms.`language_id`='$lang_id' AND FIND_IN_SET($genre_id, tbl_shortfilms.`genre_id`) and tbl_shortfilms.`movie_cost_type`='free'
            ORDER BY tbl_shortfilms.`id` DESC LIMIT $start, $limit";
      }
  ...
  else if(isset($_GET['genre']))
  {
      ...
      $query = "SELECT COUNT(*) as num FROM $tableName WHERE FIND_IN_SET(".$genre_id.", tbl_shortfilms.`genre_id`)";

      $targetpage = "manage_shortfilms.php?genre=".$genre_id; 
     ...
     $sql = "SELECT tbl_language.`language_name`, tbl_shortfilms.*
        FROM tbl_shortfilms
        LEFT JOIN tbl_language ON tbl_shortfilms.`language_id` = tbl_language.`id`
        WHERE (FIND_IN_SET(tbl_shortfilms.`genre_id`, '$genre_ids')) 
        
        ORDER BY tbl_shortfilms.`id` DESC
        LIMIT $start, $limit";
  ...
  else if(isset($_POST["search"]))
  {
    ...
    $sql="SELECT tbl_language.`language_name`,tbl_shortfilms.* FROM tbl_shortfilms
          LEFT JOIN tbl_language ON tbl_shortfilms.`language_id`= tbl_language.`id` 
          WHERE (tbl_shortfilms.`movie_title` LIKE '%$search_txt%' OR tbl_language.`language_name` LIKE '%$search_txt%') 
          ORDER BY tbl_shortfilms.`id` DESC";
  ...
  { 
    ...
    $targetpage = "manage_shortfilms.php"; 
      ...
      <div class="page_title_block">
        ...
        <div class="col-md-7 col-xs-12">              
          <div class="search_list">
            ...
            <div class="add_btn_primary"> <a href="add_shortfilm.php?add=yes">Add Shortfilm &nbsp;<i class="fa fa-plus-circle"></i></a> </div>
          </div>
        </div>
            ...
            <div class="block_wallpaper" style"width:251px;height:315px;flex-shrink: 0;border-radius:550px;">
              ...
              <div class="wall_image_title" style="background: rgba(0,0,0,0.0);border-radius:20px;">
               ...
               <ul>
                  ...
                  <li><a href="add_shortfilm.php?movie_id=<?php echo $row['id'];?>&redirect=<?=$redirectUrl?>"  style="background:linear-gradient(91.59deg, rgb(148, 63, 213) 1.37%, rgb(97, 43, 173) 98.71%);width:74px;border-radius:4px;" data-toggle="tooltip" data-tooltip="Edit"> Edit <i class="fa fa-edit"></i></a></li>
                  <?php if($row['status']!="0"){?>
...
<script type="text/javascript">
  ...
  $(".btn_delete_a").click(function(e){
      ...
      swal({
          title: "Are you sure?",
          text: "All data will be deleted of this shortfilm.",
          type: "warning",
