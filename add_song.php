<?php 
  ...
  function quality_info_data($post_id, $param, $type='song'){

    global $mysqli;

    $sql="SELECT * FROM tbl_quality WHERE `post_id` = '$post_id' AND `type` = 'song'";
  ...
  $cat_result=mysqli_query($mysqli,$cat_qry);
	...
	{	
      $title=addslashes(trim($_POST['song_title']));
      $desc=addslashes(trim($_POST['song_desc']));
      ...
      if($_POST['video_file_type']=='youtube_url'){
        ...
        $youtube_video_url = addslashes($_POST['song_url']);
      ...
      else if($_POST['video_file_type']=='server_url' OR $_POST['video_file_type']=='embedded_url')
      {
        ...
        $song_url=date('dmYhis').'_'.rand(0,99999)."_song".".".$ext;

        $tmp = $_FILES['video_local']['tmp_name'];
        
        if (move_uploaded_file($tmp, $path.$song_url)) 
        {
          $song_url=$song_url;
        } else {
      ...
      if($_POST['poster_img']=='' || $_FILES['song_poster']['error']!=4){

          // for songs poster
          $song_poster=rand(0,99999)."_".$_FILES['song_poster']['name'];
          $pic1=$_FILES['song_poster']['tmp_name'];
 
          $tpath1='images/songs/'.$song_poster; 
          copy($pic1,$tpath1);

          $thumbpath='images/songs/thumbs/'.$song_poster;
          ...
          $song_poster=date('dmYhis').'_'.rand(0,99999).".".$ext;
      ...
      // for songs cover
      $song_cover=rand(0,99999)."_".$_FILES['song_cover']['name'];
      $pic1=$_FILES['song_cover']['tmp_name'];

            
      $tpath1='images/songs/'.$song_cover; 
      copy($pic1,$tpath1);

      $thumbpath='images/songs/thumbs/'.$song_cover;
      ...
      $data = array( 
        ...
        'song_type'  =>  $_POST['video_file_type'],
        'song_title'  =>  $title,
         'release_date'  =>  $release_date,
        'total_time'  =>  $total_time,
        'director_name'  =>  $director_name,
        'maturity_rating'  =>  $maturity_rating,
        'song_cover'  =>  $mpath.$song_cover,
        'song_poster'  =>  $mpath.$song_poster,
        'song_url'  =>  $song_url,
        'video_id'  =>  $video_id,
        'song_desc'  =>  $desc,
  ...
  if(isset($_GET['song_id']))
  {  
    $qry="SELECT * FROM tbl_songs where id='".$_GET['song_id']."'";
    $result=mysqli_query($mysqli,$qry);
    $row=mysqli_fetch_assoc($result);
  }
  if(isset($_POST['submit']) and isset($_GET['song_id']))
  { 
    ...
    if($_POST['video_file_type']=='youtube_url'){
      ...
      if($_FILES['video_local']['error']!=4){
        ...
        $song_url=rand(0,99999)."_".str_replace(" ", "-", $_FILES['video_local']['name']);
    ...
    // for song poster
    if($_FILES['song_poster']['error']!=4){

      unlink('images/songs/'.$row['song_poster']);
      unlink('images/songs/thumbs/'.$row['song_poster']);

      // for song poster
      $song_poster=rand(0,99999)."_".$_FILES['song_poster']['name'];
      $pic1=$_FILES['song_poster']['tmp_name'];
            
      $tpath2='images/songs/'.$song_poster; 
      copy($pic1,$tpath2);
      ...
      $tpath2=$row['song_poster'];
    }

    // for song cover
    if($_FILES['song_cover']['error']!=4){
      unlink('images/songs/'.$row['song_cover']);
      unlink('images/songs/thumbs/'.$row['song_cover']);
    ...
    $edit=Update('tbl_songs', $data, "WHERE id = '".$_GET['song_id']."'");
    ...
    if(isset($_POST['quality_status']) && $_POST['quality_status']=='true'){
      ...
      if($_POST['video_file_type']=='local'){
        ...
        if(quality_info_data($_GET['song_id'], 'post_upload_type')=='server_url'){
          $data = array( 
            'quality_480'  =>  '',
            'quality_720'  =>  '',
            'quality_1080'  =>  '',
          ...
          $updateSql=Update('tbl_quality', $data, "WHERE `post_id` = '".$_GET['song_id']."' AND `type`='song'");
        }
        ...
        foreach($quality_arr as $key => $value) {
            ...
            if($_FILES['quality_local']['error'][$value]!=4){
                ...
                unlink('uploads/'.quality_info_data($_GET['song_id'], $param));
                ...
                $video_url=quality_info_data($_GET['song_id'], $param);
            }
        ...
        if(quality_info_data($_GET['song_id'], 'id')!=''){
          $data = array( 
            'post_upload_type'  =>  'local',
            'quality_480'  =>  $quality_480,
            'quality_720'  =>  $quality_720,
        ...
        if(quality_info_data($_GET['song_id'], 'post_upload_type')=='local'){
            
            unlink('uploads/'.quality_info_data($_GET['song_id'], 'quality_480'));
            unlink('uploads/'.quality_info_data($_GET['song_id'], 'quality_720'));
            unlink('uploads/'.quality_info_data($_GET['song_id'], 'quality_1080'));
        }
    ...
    if(isset($_GET['redirect']))
      ...
      header( "Location:add_song.php?song_id=".$_GET['song_id']);
    exit;
    ...
    <div class="card">
      <div class="page_title_block">
        ...
        <div class="col-md-7 col-xs-12" style="text-align:right;">
           <a href="manage_songs.php" style="background:white;margin-top:10px;font-size:25px;">X</a>
        </div>
      </div>
      ...
      <div class="card-body mrg_bottom">
        ...
        <form class="form form-horizontal" action="" method="post"  enctype="multipart/form-data">
          ...
          <input type="hidden" name="song_id" value="<?=(isset($_GET['song_id'])) ? $_GET['song_id'] : ''?>">

          <input type="hidden" name="is_quality" value="<?=(isset($_GET['song_id'])) ? $row['is_quality'] : ''?>">

          <input type="hidden" name="is_subtitle" value="<?=(isset($_GET['song_id'])) ? $row['subtitle'] : 'false'?>">
          ...
          <div class="section">
            <div class="section-body">
                ...
                <div class="col-md-8">
                  <select name="language_id" id="language_id" class="select2" required>
                    ...
                    <?php
                        ...
                        {
                    ?>                       
                    <option value="<?php echo $data['id'];?>" <?php if(isset($_GET['song_id']) && $row['language_id']==$data['id']){ echo 'selected'; } ?>><?php echo $data['language_name'];?></option>                          
                    <?php
                      }
                    ?>
                  </select>
                ...
                <div class="col-md-8">
                  <select name="home_cat_id" id="home_cat_id" class="select2" required>
                    ...
                    <?php
                        ...
                        {
                    ?>                       
                    <option value="<?php echo $rowhome['id'];?>" <?php if(isset($_GET['song_id']) && $row['home_cat_id']==$rowhome['id']){ echo 'selected'; } ?>><?php echo $rowhome['home_title'];?></option>                          
                    <?php
                      }
                    ?>
                  </select>
              ...
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <input type="text" name="song_title" placeholder="Enter song Title" id="song_title" value="<?php if(isset($_GET['song_id'])){echo $row['song_title'];}?>" class="form-control" required>
                </div>
              </div>
              
              
              <div class="form-group">
                <label class="col-md-3 control-label">Song Director Name:-</label>
                <div class="col-md-8">
                  <input type="text" name="director_name" placeholder="Enter Casts" id="director_name" value="<?php if(isset($_GET['song_id'])){echo $row['director_name'];}?>" class="form-control" required>
                </div>
              </div>
              ...
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <input type="text" name="cast_names" placeholder="Enter Casts" id="total_time" value="<?php if(isset($_GET['song_id'])){echo $row['cast_names'];}?>" class="form-control" required>
                </div>
              </div>
              ...
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <input type="text" name="imdb_rating" placeholder="Enter imdb rating" id="imdb_rating" value="<?php if(isset($_GET['song_id'])){echo $row['imdb_rating'];}?>" class="form-control" required>
                </div>
                  </div>
              <div class="form-group">
                <label class="col-md-3 control-label">Relsease Date :-</label>
                <div class="col-md-8">
                  <input type="date" name="release_date" placeholder="date" id="release_date" value="<?php if(isset($_GET['song_id'])){echo $row['release_date'];}?>" class="form-control" required>
                </div>
              </div>
              <div class="form-group">
                ...
                <div class="col-md-8">                       
                  <select name="video_file_type" id="video_file_type" class="select2" required>
                        ...
                        <option value="youtube_url" <?php if(isset($_GET['song_id']) && $row['song_type']=='youtube_url'){ echo 'selected'; } ?>>YouTube URL</option>
                        <option value="server_url" <?php if(isset($_GET['song_id']) && $row['song_type']=='server_url'){ echo 'selected'; } ?>>Live URL</option>
                        <option value="local" <?php if(isset($_GET['song_id']) && $row['song_type']=='local'){ echo 'selected'; } ?>>Local System</option>
                        <option value="embedded_url" <?php if(isset($_GET['song_id']) && $row['song_type']=='embedded_url'){ echo 'selected'; } ?>>Embedded URL (Open Load, Very Stream, Daily motion, Vimeo)</option>
              ...
              <div id="song_url_holder" class="form-group" style="display:none;">
                ...
                <div class="col-md-8">
                  <input type="text" name="song_url" value="<?php if(isset($_GET['song_id'])){ echo $row['song_url'];}?>" class="form-control">
                </div>
              </div>
              <div id="song_local_holder" class="form-group" style="display:none;">
                ...
                <div class="col-md-8">
                  <input type="file" name="video_local" value="" class="form-control video_local">
                    <?php 
                      if(isset($_GET['song_id']) && $row['is_quality']=='false'){
                     ?> 
                      <div style="word-break: break-all;"><label class="control-label">Current URL :-</label><?php echo $file_path.'uploads/'.$row['song_url']?></div><br>
                      <?php  
                    }
                    ?>
                    <div class="uploadPreview" style="<?php if(isset($_GET['song_id']) && $row['song_type']=='local'){ echo 'display: block;';}else{ echo 'display: none;';}?>background: #eee;text-align: center;">
                      <video height="250" width="100%" class="video-preview" src="<?php if(isset($_GET['song_id']) && $row['song_type']=='local'){ echo $file_path.'uploads/'.$row['song_url']; } ?>" controls="controls"/>
                    </div>
                </div>
              </div>
              ...
              <div id="quality_holder" style="display: none">
                ...
                <div class="form-group">
                  ...
                  <div class="col-md-8">                       
                    <select name="quality_status" id="quality_status" class="select2">
                      <option value="false" <?=(isset($_GET['song_id']) && $row['is_quality']=='false') ? 'selected="selected"' : ''?>>OFF</option>
                      <option value="true"<?=(isset($_GET['song_id']) && $row['is_quality']=='true') ? 'selected="selected"' : ''?>>ON</option>
                    </select>
                  </div>
                </div>
                <div id="quality_server_url" style="display: none;">
                  <div class="form-group">
                    ...
                    <div class="col-md-8">
                      <input type="text" name="quality_url[480]" value="<?=(isset($_GET['song_id']) && $row['is_quality']=='true') ? quality_info_data($row['id'], 'quality_480') : ''?>" class="form-control">
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="col-md-3 control-label">Enter URL for 720p :-</label>
                    <div class="col-md-8">
                      <input type="text" name="quality_url[720]" value="<?=(isset($_GET['song_id']) && $row['is_quality']=='true') ? quality_info_data($row['id'], 'quality_720') : ''?>" class="form-control">
                    </div>
                  </div>
                  <div class="form-group">
                    <label class="col-md-3 control-label">Enter URL for 1080p :-</label>
                    <div class="col-md-8">
                      <input type="text" name="quality_url[1080]" value="<?=(isset($_GET['song_id']) && $row['is_quality']=='true') ? quality_info_data($row['id'], 'quality_1080') : ''?>" class="form-control">
                    </div>
                  </div>
                </div>
                <div id="quality_local" style="display: none;">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-md-8">
                      ...
                      <?php 
                        if(isset($_GET['song_id']) && $row['is_quality']=='true' && $row['song_type']=='local' && quality_info_data($row['id'], 'quality_480')!=''){
                       ?> 
                      ...
                      ?>
                      <div class="uploadPreview" style="<?php if(isset($_GET['song_id']) && $row['song_type']=='local' && $row['is_quality']=='true' && quality_info_data($row['id'], 'quality_480')!=''){ echo 'display: block;';}else{ echo 'display: none;';}?>background: #eee;text-align: center;">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-md-8">
                      ...
                      <?php 
                        if(isset($_GET['song_id']) && $row['is_quality']=='true' && $row['song_type']=='local' && quality_info_data($row['id'], 'quality_720')!=''){
                       ?> 
                      ...
                      ?>
                      <div class="uploadPreview" style="<?php if(isset($_GET['song_id']) && $row['song_type']=='local' && $row['is_quality']=='true' && quality_info_data($row['id'], 'quality_720')!=''){ echo 'display: block;';}else{ echo 'display: none;';}?>background: #eee;text-align: center;">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-md-8">
                      ...
                      <?php 
                        if(isset($_GET['song_id']) && $row['is_quality']=='true' && $row['song_type']=='local' && quality_info_data($row['id'], 'quality_1080')!=''){
                       ?> 
                      ...
                      ?>
                      <div class="uploadPreview" style="<?php if(isset($_GET['song_id']) && $row['song_type']=='local' && $row['is_quality']=='true' && quality_info_data($row['id'], 'quality_1080')!=''){ echo 'display: block;';}else{ echo 'display: none;';}?>background: #eee;text-align: center;">
              ...
              <div class="subtitle" style="margin: 10px 0px;display: none;">
                <div class="form-group">
                  ...
                  <div class="col-md-8">                       
                    <select name="subtitle" id="subtitle" class="select2">
                      <option value="false" <?=(isset($_GET['song_id']) && $row['subtitle']=='false') ? 'selected="selected"' : ''?>>OFF</option>
                      <option value="true"<?=(isset($_GET['song_id']) && $row['subtitle']=='true') ? 'selected="selected"' : ''?>>ON</option>
                    </select>
                  </div>
                </div>
                ...
                <?php }else if(isset($_GET['song_id']))
                {

                ?>
                <div class="subtitle_fields" style="display: none">
                <?php
                    $sql_subtitle="SELECT * FROM tbl_subtitles WHERE post_id='".$row['id']."' AND `type`='song'";
              ...
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <input type="text" name="total_time" placeholder="Enter video time" id="total_time" value="<?php if(isset($_GET['song_id'])){echo $row['total_time'];}?>" class="form-control" required>
                </div>
              </div>
               <div class="form-group">
                <label class="col-md-3 control-label">Maturity rating :-</label>
                <div class="col-md-8">
                  <input type="text" name="maturity_rating" placeholder="Enter maturity rating" id="maturity_rating" value="<?php if(isset($_GET['song_id'])){echo $row['maturity_rating'];}?>" class="form-control" required>
                </div>
              </div>
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <div class="fileupload_block" style="border-radius:10px;height:189px;">
                    <input type="file" name="song_poster" value="" accept=".png, .jpg, .jpeg, .svg, .gif" <?php echo (!isset($_GET['song_id'])) ? 'required="require"' : '' ?> id="fileupload">
                    <div class="fileupload_img">
                      <?php 
                        $img_src="";

                        if(!isset($_GET['song_id']) || !file_exists($row['song_poster'])){
                          $img_src='assets/images/browse.svg';
                          echo '<input type="hidden" name="poster_img" value="">';
                        }else{
                          $img_src=$row['song_poster'];
                        }
              ...
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <div class="fileupload_block" style="border-radius:10px;height:189px;">
                    <input type="file" name="song_cover" value="" accept=".png, .jpg, .jpeg, .svg, .gif" <?php echo (!isset($_GET['song_id'])) ? 'required="require"' : '' ?> id="fileupload" style="margin-top: 5%;">
                    <div class="fileupload_img">
                      <?php 
                        $img_src="";

                        if(!isset($_GET['song_id']) || !file_exists($row['song_cover'])){
                          $img_src='assets/images/browse.svg';  

                        }else{
                          $img_src=$row['song_cover'];
                        }
              ...
              <div class="form-group">
                ...
                <div class="col-md-8">
                  <textarea name="song_desc" id="song_desc" rows="5" class="form-control"><?php if(isset($_GET['song_id'])){ echo $row['song_desc']; } ?></textarea>
                  <script>
                    CKEDITOR.replace('song_desc');
                  </script>
                </div>
              </div>
              ...
              <div class="form-group">
                <div class="col-md-9 col-md-offset-3">
                  <button type="submit" name="submit" style="border-radius:10px;" class="btn btn-primary"><?php if(isset($_GET['song_id'])){ ?> Update <?php }else{?>Save<?php } ?> </button>
                </div>
              </div>
            </div>
          </div>
...
<script type="text/javascript">
  ...
  function readURL(input) {
    if (input.files && input.files[0]) {
      ...
      reader.onload = function(e) {
        $("input[name='song_poster']").next(".fileupload_img").find("img").attr('src', e.target.result);
      }
      
      reader.readAsDataURL(input.files[0]);
    }
  }
  $("input[name='song_poster']").change(function() { 
  ...
  function readURL1(input) {
    if (input.files && input.files[0]) {
      ...
      reader.onload = function(e) {
        $("input[name='song_cover']").next(".fileupload_img").find("img").attr('src', e.target.result);
      }
  ...
  $("input[name='song_cover']").change(function() { 
    var file=$(this);
  ...
  $("#video_file_type").on("change",function(e){
    ...
    if(_type=='youtube_url' || _type=='server_url' || _type=='embedded_url'){
      $("#song_url_holder").show();
      $("input[name='video_local']").attr("required",false);
      $("input[name='song_url']").attr("required",true);
      $("#song_local_holder").hide();

    }
    else if(_type=='local'){
      $("input[name='song_url']").attr("required",false);
      $("input[name='video_local']").attr("required",true);
      $("#song_local_holder").show();
      $("#song_url_holder").hide();
    }
    else{
      $("#quality_holder").hide();
      $("#song_local_holder").hide();
  ...
  $("select[name='subtitle_type[]']").change(function(e){
    ...
    if(val==='live_url'){
      ...
      if($("input[name='song_id']").val()=='' && $(this).parents(".subtitle_container").find("input[name='old_subtitle_type']").val()!='local'){
        ...
        if(subtitle_type==''){
          ...
          if(subtitle_type=='live_url'){
            ...
            if($(this).parents('.moreContainer').html()==undefined){
                ...
                if(file[0].files.length == 0 && ($("input[name='song_id']").val()=='' || $("input[name='is_subtitle']").val()=='false')){
  ...
  $(".removeSubtitle").click(function(event){
    ...
    var post_id=$("input[name='song_id']").val();
    ...
    if(confirm("Do you really want to remove this subtitle?")){
        $.ajax({
          ...
          url:'processData.php',
          dataType:'json',
          data:{id:id,'action':'remove_subtitle','type':'song','post_id':post_id},
  ...
  $(".btn_fetch").click(function(e){
    ...
    if(imdb_id_title==''){
      ...
      $.ajax({
        ...
        url:'processImdb.php',
        dataType:'json',
        data:{'action':'getsongDetails', 'id' : imdb_id_title},
        success:function(res){
            ...
            if(res.status=='1'){
                ...
                $("input[name='song_poster']").attr("required",false);
                $("input[name='song_title']").val(res.title);
                ...
                $("textarea[name='song_desc']").val(res.plot);
                CKEDITOR.instances['song_desc'].setData(res.plot);
            }
          }
      });
