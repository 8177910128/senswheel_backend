<?php 
  ...
  if(isset($_GET['language']))
  {
      ...
      if(isset($_GET['genre'])){
        $query = "SELECT COUNT(*) as num FROM $tableName WHERE tbl_movies.`language_id`='$lang_id' AND FIND_IN_SET(".$_GET['genre'].", tbl_movies.`genre_id`) and tbl_movies.`movie_cost_type`='free'";
      ...
      $sql="SELECT tbl_language.`language_name`,tbl_movies.* FROM tbl_movies
          ...
          WHERE tbl_movies.`language_id`='$lang_id' and tbl_movies.`movie_cost_type`='free'
      ...
      if(isset($_GET['genre'])){
        ...
        $sql="SELECT tbl_language.`language_name`,tbl_movies.* FROM tbl_movies
            LEFT JOIN tbl_language ON tbl_movies.`language_id`= tbl_language.`id` 
            WHERE tbl_movies.`language_id`='$lang_id' AND FIND_IN_SET($genre_id, tbl_movies.`genre_id`) and tbl_movies.`movie_cost_type`='free'
            ORDER BY tbl_movies.`id` DESC LIMIT $start, $limit";
      }
  ...
  else if(isset($_GET['genre']))
  {
     ...
     $sql = "SELECT tbl_language.`language_name`, tbl_movies.*
        FROM tbl_movies
        LEFT JOIN tbl_language ON tbl_movies.`language_id` = tbl_language.`id`
        WHERE (FIND_IN_SET(tbl_movies.`genre_id`, '$genre_ids')) 
        AND tbl_movies.`movie_cost_type` = 'free'
  ...
  else if(isset($_POST["search"]))
  {
    ...
    $sql="SELECT tbl_language.`language_name`,tbl_movies.* FROM tbl_movies
          LEFT JOIN tbl_language ON tbl_movies.`language_id`= tbl_language.`id` 
          WHERE (tbl_movies.`movie_title` LIKE '%$search_txt%' OR tbl_language.`language_name` LIKE '%$search_txt%') and tbl_movies.`movie_cost_type`='free'
          ORDER BY tbl_movies.`id` DESC";
  ...
  { 
    ...
    $sql="SELECT tbl_language.`language_name`,tbl_movies.* FROM tbl_movies
          LEFT JOIN tbl_language ON tbl_movies.`language_id`=tbl_language.`id` where    tbl_movies.`movie_cost_type`='free'
          ORDER BY tbl_movies.`id` DESC LIMIT $start, $limit";
  }
      ...
      <div class="page_title_block">
        ...
        <div class="col-md-7 col-xs-12">              
          <div class="search_list">
            ...
            <div class="add_btn_primary"> <a href="add_movie.php?add=yes">Add Movie &nbsp;<i class="fa fa-plus-circle"></i></a> </div>
          </div>
        </div>
      ...
      <div class="col-md-12 mrg-top">
        <div class="row">
          ...
          <div class="col-lg-3 col-sm-6 col-xs-12" style="border-radius:20px;">
            <div class="block_wallpaper" style"width:300px;height:315px;flex-shrink: 0;border-radius:550px;">
              ...
              <div class="wall_image_title" style="background: rgba(0,0,0,0.0);border-radius:20px;">
               ...
               <ul>
                  ...
                  <li><a href="add_movie.php?movie_id=<?php echo $row['id'];?>&redirect=<?=$redirectUrl?>"  style="background:linear-gradient(91.59deg, rgb(148, 63, 213) 1.37%, rgb(97, 43, 173) 98.71%);width:74px;border-radius:4px;" data-toggle="tooltip" data-tooltip="Edit"> Edit <i class="fa fa-edit"></i></a></li>
                  <?php if($row['status']!="0"){?>
